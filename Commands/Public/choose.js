module.exports = (bot, db, config, winston, userDocument, serverDocument, channelDocument, memberDocument, msg, suffix, commandData) => {
	...
	if(suffix && choices.length>=2) {
		...
		msg.channel.createMessage(`${msg.author.mention} I couldn't get that. Make sure to use the syntax \`${bot.getCommandPrefix(msg.guild, serverDocument)}${commandData.name} ${commandData.usage}\``);
	}
};
