...
class DiscordBot {
  ...
  initialize () {
    ...
    // Register commands
    this.bot.registry
      .registerGroup('roconnect', 'RoConnect')
  ...
  async message (message) {
    ...
    if (server.getSetting('verifyChannel') === message.channel.id && message.cleanContent.toLowerCase() !== message.guild.commandPrefix + 'verify' && !(this.bot.isOwner(message.author) || message.member.hasPermission('MANAGE_GUILD') || message.member.roles.find(role => role.name === 'Kosh Admin'))) {
      message.delete()
  ...
  setActivity (text, activityType) {
    this.bot.user.setActivity(text || 'http://koshsystems.org/rokosh', { type: activityType })
  }
