...
class DiscordMember {
  ...
  async verify (options) {
    ...
    if (this.user.bot) {
      return status({
        status: false,
        error: ':robot: Kosh RoConnect cannot verify bots.'
      })
    }
    ...
    if (this.member.roles.find(role => role.name === 'RoVer Bypass')) {
      return status({
        status: false,
        error: ':octagonal_sign: Kosh RoConnect cannot act on users with the "Kosh Bypass" role.',
        nonFatal: true
      })
    }

    // Create a warning to append to any errors. In some permission setups, Kosh RoConnect is reliant on role positioning (specifically if it has administrator or not)
    if ((await this.server.members.fetch(this.bot.user.id)).manageable) {
      errorAppend = "\n\nRoConnect's position in the role list is below that of this user. With certain setups, this will prevent Kosh RoConnect from working correctly. Please have a server admin drag Kosh's role above all other roles in order to fix this problem."
    }
        ...
        if (!this.member.roles.has(role) && this.server.roles.has(role)) {
          try {
            ...
            return status({
              status: false,
              nonFatal: true,
              error: "Kosh RoConnect doesn't have permissions to add roles to that user." + errorAppend
            })
          }
        }
      }
        ...
        if (this.member.roles.has(role) && this.server.roles.has(role)) {
          try {
            ...
            return status({
              status: false,
              nonFatal: true,
              error: "Kosh RoConnect doesn't have permission to remove roles from that user." + errorAppend
            })
          }
        }
      }
        ...
        if (this.member.displayName !== nickname) {
          try {
            ...
            return status({
              status: false,
              nonFatal: true,
              error: this.member.guild.ownerID === this.member.id ? "Sorry, Kosh RoConnect can't change the server owner's nickname due to Discord permission restrictions. Please manually update your nickname. Or don't, I'm just an error message." : "Kosh RoConnect doesn't have permission to change that user's nickname." + errorAppend
            })
          }
        }
      }
          ...
          if (options.message) {
            ...
            if (VerificationAttempts.get(this.id) > 1) {
              error =
                ...
                - Try visiting https://koshsystems/rokosh/ in an incognito / private browser window. (It's possible you are signed into the wrong Discord account in your browser)
