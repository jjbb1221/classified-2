...
class NicknameFormatCommand extends Command {
  constructor (client) {
    super(client, {
      ...
      description: "`<format>` Set the format Kosh RoConnect will use when setting users' nicknames. Available replacements are %USERNAME%, %RANK%, %USERID%, %SERVER%, %DISCORDNAME%, and %DISCORDID%. Example:` %USERNAME% - (%USERID%)`. Default `%USERNAME%`.",
